...
# 2. HUD STYLING (NAVY LOCK + EXPANDED TOOLTIPS)
st.markdown("""
    <style>
    ...
    /* MEGA-TOOLTIP FIX: LARGE BOX, 14px FONT, WRAPPING ENABLED */
    ...
    .leaflet-popup-content { width: 550px !important; margin: 0 !important; }
    .leaflet-popup-tip { background: #002366 !important; }
    </style>
    ...
    for iata, data in bundle.items():
        ...
        if t.data:
            for line in t.data.forecast:
                ...
                if (line.visibility.value if line.visibility else 9999) < v_lim: l_issues.append("VIS")
                ...
                if l_issues:
                    w_issues = l_issues; w_time = f"{line.start_time.dt.strftime('%H')}Z"; f_wind_spd, f_wind_dir = max(l_spd, l_gst), l_dir; w_prob = ("PROB" in l_raw)
...
for iata, info in base_airports.items():
    ...
    # MEGA-HOVER CONTENT
    shared_content = f"""
    <div style="width:530px; padding:20px; background:white; color:#002366; line-height:1.4;">
        <div style="border-bottom:3px solid #d6001a; margin-bottom:15px; display:flex; justify-content:space-between;">
            <b style="font-size:20px;">{iata} STATUS {trend_icon}</b>
            <span style="font-size:14px; font-weight:bold;">{time_horizon}</span>
        </div>
        <div style="background:#f0f2f6; padding:12px; border-radius:6px; border-left:8px solid {color}; margin-bottom:15px;">
            <b style="font-size:16px;">{rwy_text} X-Wind: {xw} KT</b><br>
            <b style="font-size:15px;">ACTUAL:</b> <span style="color:{'#d6001a' if m_issues else '#008000'};">{"/".join(m_issues) if m_issues else "STABLE"}</span><br>
            <b style="font-size:15px;">FORECAST:</b> <span style="color:{'#eb8f34' if data['f_issues'] else '#008000'};">{"/".join(data['f_issues']) if data['f_issues'] else "NIL"}</span>
        </div>
        <div style="font-size:14px; background:#f9f9f9; padding:10px; border:1px solid #ddd; margin-bottom:10px; word-wrap: break-word;">
            <b>METAR:</b><br>{data['raw_m']}
        </div>
        <div style="font-size:14px; background:#f9f9f9; padding:10px; border:1px solid #ddd; word-wrap: break-word;">
            <b>TAF:</b><br>{data['raw_t']}
        </div>
    </div>
    """
...
for mkr in map_markers:
    folium.CircleMarker(
        location=[mkr['lat'], mkr['lon']], radius=8, color=mkr['color'], fill=True, 
        popup=folium.Popup(mkr['content'], max_width=550), 
        tooltip=folium.Tooltip(mkr['content'], direction='top', sticky=False)
    ).add_to(m)
st_folium(m, width=1200, height=1200, key="map_mega_v284")

# 10. ALERTS & STRATEGY (RESTORED FROM v28.2)
...
# 11. STRATEGY BRIEF & ALTERNATE ENGINE (RESTORED)
if st.session_state.investigate_iata != "None":
    ...
    for g in green_stations:
        if g != iata:
            ...
            alt_data = weather_data.get(g)
            alt_xw = calculate_xwind(alt_data['f_wind_dir'] or alt_data['w_dir'], alt_data['f_wind_spd'] or alt_data['w_spd'], base_airports[g]['rwy'])
...
st.text_area("Handover Report:", value=h_txt, height=200, key="handover_log_v284", label_visibility="collapsed")
