...
# 2. HUD STYLING (NAMESPACED FOR STRATEGY BRIEF)
st.markdown("""
    <style>
    /* GLOBAL THEME */
    ...
    [data-testid="stSidebar"] label p, [data-testid="stSidebar"] p, [data-testid="stSidebar"] span { 
...
# 10. UI RENDER
st.markdown(f'<div class="ba-header"><div>OCC HUD v31.1</div><div>{datetime.now().strftime("%H:%M")} UTC</div></div>', unsafe_allow_html=True)
...
st_folium(m, width=1200, height=1200, key="map_restored_v311")
...
st.text_area("Log:", value=h_txt, height=200, key="log_v311", label_visibility="collapsed")import streamlit as st
import folium
from streamlit_folium import st_folium
from avwx import Metar, Taf
import math
...
# 2. HUD STYLING (v29.2 BASE + DROPDOWN & BRIEF LOCK)
st.markdown("""
    <style>
    .main { background-color: #001a33 !important; }
    html, body, [class*="st-"], div, p, h1, h2, h4, label { color: white !important; }
    ...
    /* STRATEGY BRIEF AGGRESSIVE FONT LOCK */
    .reason-box { 
        background-color: #ffffff !important; 
        border: 1px solid #ddd; padding: 25px; border-radius: 5px; 
        margin-top: 20px; border-top: 10px solid #d6001a; 
    ...
    .reason-box li { color: #002366 !important; }
...
def process_weather_for_horizon(bundle, airport_dict, horizon_limit, xw_threshold):
    ...
    for iata, data in bundle.items():
        ...
        processed[iata] = {
            "vis": m_vis, "status": "online", "w_dir": m_w_dir, "w_spd": m_w_spd, "w_gst": m_w_gst,
...
for iata, info in base_airports.items():
    ...
    if not actual_haz and not fore_haz: green_stations.append(iata)
    if info.get('hide_map') or info.get('fleet') == "Special": continue
    if not ((info['fleet'] == "Cityflyer" and show_cf) or (info['fleet'] == "Euroflyer" and show_ef)): continue
...
# 10. UI RENDER
st.markdown(f'<div class="ba-header"><div>OCC HUD v30.9</div><div>{datetime.now().strftime("%H:%M")} UTC</div></div>', unsafe_allow_html=True)
m = folium.Map(location=[50.0, 10.0], zoom_start=4, tiles=("CartoDB dark_matter" if map_theme == "Dark Mode" else "CartoDB positron"), scrollWheelZoom=False)
for mkr in map_markers:
    folium.CircleMarker(location=[mkr['lat'], mkr['lon']], radius=7, color=mkr['color'], fill=True, popup=folium.Popup(mkr['content'], max_width=650, auto_pan=True, auto_pan_padding=(150, 150)), tooltip=folium.Tooltip(mkr['content'], direction='top', sticky=False)).add_to(m)
st_folium(m, width=1200, height=1200, key="map_MLA_SN_fix")
...
h_txt = f"HANDOVER {datetime.now().strftime('%H:%M')}Z\n" + "="*50 + "\n"
for i_ata, d_taf in taf_alerts.items(): h_txt += f"{i_ata}: {d_taf['type']}\n"
st.text_area("Log:", value=h_txt, height=200, key="log_v309", label_visibility="collapsed")
