...
st.markdown("""
    ...
    .limits-table { width: 100%; font-size: 0.85rem; border-collapse: collapse; margin-top: 10px; color: white !important; background-color: rgba(255,255,255,0.1); }
    .limits-table td, .limits-table th { border: 1px solid rgba(255,255,255,0.3); padding: 8px; text-align: left; }
...
def bold_hazard(text):
    ...
    text = re.sub(r'(\b(0\d{3}|1[0-4]\d{2})\b)', r'<b>\1</b>', text)
...
base_airports = {
    ...
    "EXT": {"icao": "EGTE", "lat": 50.734, "lon": -3.414, "rwy": 260, "fleet": "Cityflyer", "spec": False},
    ...
    "BCN": {"icao": "LEBL", "lat": 41.297, "lon": 2.083, "rwy": 70, "fleet": "Euroflyer", "spec": False},
...
# 7. DATA FETCH
now = datetime.now()
sync_key = now.strftime('%Y%m%d%H') + str(0 if now.minute < 30 else 30)
...
for iata, info in base_airports.items():
    ...
    if data['status'] == "online":
        ...
        # Buffer Logic: Global 20kt Amber, 25kt Red
        if xw >= 25: m_issues.append(f"X-WIND {xw}kt"); color = "#d6001a"
        elif xw >= 20: m_issues.append(f"X-WIND {xw}kt"); color = "#eb8f34"
        
        if is_shown or info['fleet'] == "Alternate":
            if m_issues: 
                if color == "#008000": color = "#eb8f34"
                if any(x in str(m_issues) for x in ["VIS", "CLOUD", "FZRA"]) or xw >= 25: color = "#d6001a"
    ...
    if is_shown:
        ...
        popup_html = f"""<div style="width:600px; color:black !important; font-family:sans-serif; font-size:16px; line-height:1.4;"><b style="color:#002366; font-size:20px; border-bottom:2px solid #d6001a; display:block; padding-bottom:5px; margin-bottom:10px;">{iata} STATUS</b><div style="margin-top:5px; padding:12px; border-left:8px solid {color}; background:#f4f4f4; border-radius:4px;"><b style="color:#002366; font-size:18px;">RWY {rwy_str} Live X-Wind:</b> <span style="color:{'#d6001a' if xw >= 25 else '#eb8f34' if xw >= 20 else '#002366'}; font-weight:900; font-size:20px;">{xw} KT</span><br><div style="margin-top:8px;"><b>ACTUAL:</b> <span style="color:#d6001a; font-weight:bold;">{actual_str}</span><br><b>FORECAST:</b> <span style="color:#eb8f34; font-weight:bold;">{forecast_str}</span></div></div><hr><div><b>METAR:</b><br>{bold_hazard(data['raw_m'])}<br><br><b>TAF:</b><br>{bold_hazard(data['raw_t'])}</div></div>"""
...
st_folium(m, width=1000, height=1000, key="map_v145")
...
if st.session_state.investigate_iata != "None":
    ...
    elif "X-WIND" in issue_desc: impact = f"Crosswind at limits ({xw_val}kt). Verify runway state."
    ...
    st.markdown(f"""<div class="reason-box"><h3>{iata} Strategy Brief: {issue_desc} <span class="copy-btn" onclick="tacticalCopy('{urllib.parse.quote(brief_copy)}')">📋</span></h3><p><b>WX Summary:</b> Live crosswind <b>{xw_val}kt</b> for RWY {info.get('rwy')}°.</p><p><b>Impact:</b> {impact}</p><p style="color:#d6001a !important; font-size:1.1rem;"><b>✈️ Strategic Alternate:</b> {alt_iata} {alt_note}</p><hr><div style="display:flex; gap:20px;"><div style="flex:1;"><b>METAR:</b><br><small>{bold_hazard(d.get('raw_m'))}</small></div><div style="flex:1;"><b>TAF:</b><br><small>{bold_hazard(d.get('raw_t'))}</small></div></div></div>""", unsafe_allow_html=True)
...
st.markdown(f'<div class="section-header">📝 Shift Handover Log <span class="copy-btn" onclick="tacticalCopy(\'{urllib.parse.quote(h_txt_clean)}\')">📋</span></div>', unsafe_allow_html=True)
st.text_area("Handover Report:", value=h_txt_clean, height=200, label_visibility="collapsed")
