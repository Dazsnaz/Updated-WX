...
# 1. PAGE CONFIG
st.set_page_config(layout="wide", page_title="BA OCC HUD v3", page_icon="✈️")
...
st.markdown("""
    ...
    .floating-stats {
        ...
        border: 1px solid #005a9c; min-width: 380px;
    }
    .floating-alerts {
        position: absolute; top: 130px; right: 20px; z-index: 1000;
        background: rgba(0, 35, 102, 0.85); padding: 15px; border-radius: 8px;
...
if ui_visible:
    ...
    <div class="floating-stats">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <span style="background:#d6001a; padding:2px 8px; border-radius:10px; font-size:10px;">{trend_icon}</span>
        </div>
        <hr style="margin:5px 0; border:0.5px solid #005a9c;">
        <div style="display:flex; justify-content:space-between; font-size:12px;">
            <div><b>CF:</b> {counts['Cityflyer']['G']}G|{counts['Cityflyer']['A']}A|{counts['Cityflyer']['R']}R</div>
            <div><b>EF:</b> {counts['Euroflyer']['G']}G|{counts['Euroflyer']['A']}A|{counts['Euroflyer']['R']}R</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.investigate_iata in active_alerts and ui_visible:
    ...
    if st.session_state.minimized:
        ...
        st.markdown(f"""<div class="floating-analysis"><h3>{st.session_state.investigate_iata} IMPACT ANALYSIS</h3><p><b>DETECTION:</b> {d['reason']} identified. Local weather currently below standard minima.</p><p><b>CONSEQUENCE:</b> This forecast may cause diversions or ATC slots. Long delays expected to the operation if conditions persist.</p><hr><p style="font-size:11px; background:#f4f4f4; padding:8px;"><b>TAF:</b> {d['t']}</p></div>""", unsafe_allow_html=True)
