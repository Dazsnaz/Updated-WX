...
st.markdown("""
    ...
    [data-testid="stFileUploader"] section { 
        background-color: #005a9c !important; border: 1px solid white !important; 
...
# 7. DATA FETCH & PROCESSING (Using Dynamically Filtered Stations)
    ...
    for iata, data in bundle.items():
        ...
        if t and t.data and hasattr(t.data, 'forecast'):
            for line in t.data.forecast:
                ...
                # SAFE FALLBACK FOR WIND/GUST VALUES
...
# 8. UI LOOP & INBOUND FLIGHT INJECTION
...
for iata, info in display_airports.items():
    ...
    # ---------------------------------------------------------
    # CSV FLIGHT INJECTION LOGIC (Hides past flights)
    # ---------------------------------------------------------
    inbound_html = ""
    if not flight_schedule.empty:
...
def live_radar_map(cf_on, ef_on, scheduler):
    ...
    for mkr in map_markers:
        folium.CircleMarker(
            ...
            radius=7, color=mkr['color'], fill=True, 
    ...
    if show_radar:
        ...
        for p in planes:
            ...
            if call in scheduler:
                flt = str(scheduler[call]['FLT'])
                dep = str(scheduler[call]['DEP'])
                arr = str(scheduler[call]['ARR'])
            ...
            folium.Marker(
                [p['lat'], p['lon']], 
                icon=folium.DivIcon(html=icon_svg, class_name="dummy"), 
    ...
    map_res = st_folium(m, width=1200, height=800, key="map_stable_v292", returned_objects=["center", "zoom"])
