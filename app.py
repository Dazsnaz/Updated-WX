...
# 2. HUD STYLING (EXACT v29.2 FONT SYNC + v31.8 FORCE-NAVY)
st.markdown("""
    <style>
    /* GLOBAL THEME */
    ...
    /* ALERT TABS (v29.2 COLORS) */
    button[kind="secondary"] { background-color: #eb8f34 !important; color: white !important; border: 1px solid white !important; }
    button[kind="primary"] { background-color: #d6001a !important; color: white !important; border: 1px solid white !important; }

    /* STRATEGY BRIEF - v31.8 FORCE NAVY */
    .reason-box { 
        background-color: #ffffff !important; 
        border: 1px solid #ddd; padding: 25px; border-radius: 5px; 
        margin-top: 20px; border-top: 10px solid #d6001a; 
    }
    /* Targeted class for all internal text components */
    .navy-lock { color: #002366 !important; }
    .navy-lock * { color: #002366 !important; }
    ...
    .alt-table { width: 100%; border-collapse: collapse; margin-top: 10px; color: #002366 !important; }
...
# 10. UI RENDER
st.markdown(f'<div class="ba-header"><div>OCC HUD v31.8</div><div>{datetime.now().strftime("%H:%M")} UTC</div></div>', unsafe_allow_html=True)
...
st_folium(m, width=1200, height=1200, key="map_stable_v318")
...
if st.session_state.investigate_iata != "None":
    ...
    st.markdown('<div class="reason-box navy-lock">', unsafe_allow_html=True)
    st.markdown(f'<h3>{iata} Strategy Brief</h3>', unsafe_allow_html=True)
    ...
    with col_l:
        st.markdown(f'<p><b>Active Hazards:</b> {haz_txt}</p>', unsafe_allow_html=True)
        st.markdown(f'<p><b>Selected {sel_rwy} Crosswind:</b> <b>{final_xw}kt</b>.</p>', unsafe_allow_html=True)
        st.markdown('<p><b>Strategic Alternate Recommendations:</b></p>', unsafe_allow_html=True)
        ...
        for g in green_stations[:3]:
            alt_html += f'<tr><td><b>{g}</b></td><td>{calculate_dist(info["lat"], info["lon"], base_airports[g]["lat"], base_airports[g]["lon"])} NM</td><td>{weather_data[g].get("w_spd",0)} kt</td><td>HIGH</td></tr>'
        alt_html += '</table>'
        st.markdown(alt_html, unsafe_allow_html=True)
        
    with col_r:
        st.markdown('<b>LIVE METAR</b>', unsafe_allow_html=True)
        st.markdown(f'<div class="wx-box">{bold_hazard(d.get("raw_m"))}</div>', unsafe_allow_html=True)
        st.markdown('<b>LIVE TAF</b>', unsafe_allow_html=True)
...
st.text_area("Log:", value=h_txt, height=200, key="log_v318", label_visibility="collapsed")
