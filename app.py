...
# 2. HUD STYLING (V29.2 BASE + V31.9 COMPONENT LOCKS)
st.markdown("""
    <style>
    /* GLOBAL THEME */
...
# 10. UI RENDER
st.markdown(f'<div class="ba-header"><div>OCC HUD v31.9</div><div>{datetime.now().strftime("%H:%M")} UTC</div></div>', unsafe_allow_html=True)
...
st_folium(m, width=1200, height=1200, key="map_stable_v319")
...
# 12. STRATEGY BRIEF (v31.9 HARD-CODED HTML INJECTION)
if st.session_state.investigate_iata != "None":
    ...
    haz_txt = (taf_alerts.get(iata, {}) or metar_alerts.get(iata, {}) or {}).get('type', 'STABLE')
    
    st.markdown('<div class="reason-box">', unsafe_allow_html=True)
    st.markdown(f'<div style="color:#002366 !important; font-size:24px; font-weight:900; margin-bottom:20px;">{iata} Strategy Brief</div>', unsafe_allow_html=True)
    
    # Selection Toggle
    st.markdown(f'<div style="color:#002366 !important; font-weight:bold; margin-bottom:5px;">Manual Runway Selection:</div>', unsafe_allow_html=True)
    ...
    with col_l:
        st.markdown(f'<div style="color:#002366 !important; font-size:16px;"><b>Active Hazards:</b> {haz_txt}</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="color:#002366 !important; font-size:16px;"><b>Selected {sel_rwy} Crosswind:</b> <b>{final_xw}kt</b>.</div>', unsafe_allow_html=True)
        st.markdown('<div style="color:#002366 !important; font-size:18px; font-weight:bold; margin-top:20px;">Strategic Alternate Recommendations:</div>', unsafe_allow_html=True)
        
        # Build Table manually to force colors
        alt_html = '<table class="alt-table"><tr><th>Alternate</th><th>Dist (NM)</th><th>Best XW</th><th>Status</th></tr>'
        # Basic logic check for preferred alts
        pref_alts = {"FLR":["PSA","BLQ"], "INN":["MUC"], "FNC":["PSO","FAO","TFS"]}.get(iata, [])
        found_alts = 0
        for p in pref_alts:
            if p in base_airports and p in weather_data:
                ...
                alt_html += f'<tr><td style="color:#002366 !important;"><b>{p}</b></td><td style="color:#002366 !important;">{dist} NM</td><td style="color:#002366 !important;">{xw} kt</td><td style="color:#d6001a !important; font-weight:bold;">PREF</td></tr>'
                found_alts += 1
        for g in green_stations:
            if found_alts >= 3: break
            if g != iata and g not in pref_alts:
                ...
                alt_html += f'<tr><td style="color:#002366 !important;"><b>{g}</b></td><td style="color:#002366 !important;">{dist} NM</td><td style="color:#002366 !important;">{xw} kt</td><td style="color:#008000 !important; font-weight:bold;">OK</td></tr>'
                found_alts += 1
        alt_html += '</table>'
        st.markdown(alt_html, unsafe_allow_html=True)
        
    with col_r:
        st.markdown('<div style="color:#002366 !important; font-weight:bold; margin-bottom:5px;">LIVE METAR</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="wx-box">{bold_hazard(d.get("raw_m"))}</div>', unsafe_allow_html=True)
        st.markdown('<div style="color:#002366 !important; font-weight:bold; margin-bottom:5px;">LIVE TAF</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="wx-box">{bold_hazard(d.get("raw_t"))}</div>', unsafe_allow_html=True)
...
st.text_area("Log:", value=h_txt, height=200, key="log_v319", label_visibility="collapsed")
