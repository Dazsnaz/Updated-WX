...
# 2. THE STABLE CSS (Locked to v29.2)
st.markdown("""
    <style>
...
# 4. DATA LOGIC (STABLE 47+ STATIONS)
base_airports = {
    ...
    "PSA": {"icao": "LIRP", "lat": 43.683, "lon": 10.395, "rwy": 220, "fleet": "Special"},
    "MUC": {"icao": "EDDM", "lat": 48.353, "lon": 11.786, "rwy": 260, "fleet": "Special"},
}
...
# 8. PROCESSOR (BUG-FREE)
def process_simple(bundle, airport_dict):
    proc = {}
...
weather_data = process_simple(weather_bundle, base_airports)
...
for iata, info in base_airports.items():
    ...
    color = "#008000"
    if xw >= 25: color = "#d6001a"; metar_alerts[iata] = "XWIND"
    
    if info['fleet'] == "Special": continue
    if not ((info['fleet'] == "Cityflyer" and show_cf) or (info['fleet'] == "Euroflyer" and show_ef)): continue

    content = f"""<div style="color:black !important; background:white; padding:10px;"><b>{iata}</b><br>X-Wind: {xw}KT</div>"""
    markers.append({"lat": info['lat'], "lon": info['lon'], "color": color, "content": content})

# 10. RENDER
st.markdown(f'<div class="ba-header"><div>OCC HUD v33.4 (Reset)</div><div>{datetime.now().strftime("%H:%M")}Z</div></div>', unsafe_allow_html=True)
...
# 11. STRATEGY BRIEF (SIMPLE SEQUENTIAL LAYOUT - NO BUGS)
if st.session_state.investigate_iata != "None":
    ...
    # Simple Radio button with forced styling
    st.markdown('<p style="color:#002366 !important; font-weight:bold;">Runway Override:</p>', unsafe_allow_html=True)
    r1 = int(info['rwy']/10)
    sel_rwy = st.radio("Toggle", [f"RWY {r1:02d}", f"RWY {((r1+18)%36):02d}"], horizontal=True, key="rwy_toggle", label_visibility="collapsed")
    
    st.markdown(f"""
        <p style="color:#002366 !important; font-size:18px;"><b>Actual METAR:</b><br>{bold_hazard(d['raw_m'])}</p>
        <p style="color:#002366 !important; font-size:18px;"><b>Actual TAF:</b><br>{bold_hazard(d['raw_t'])}</p>
        <hr>
        <p style="color:#002366 !important;">Strategic Action: Monitor crosswind trends for {sel_rwy}.</p>
