...
# 2. HUD STYLING (V24.7 - REINFORCED NAVY-TEXT LOCK)
st.markdown("""
    <style>
    /* GLOBAL BACKGROUND & BASE WHITE TEXT */
    ...
    /* DROPDOWN FIX (FORCE NAVY ON WHITE) */
    ...
    /* STRATEGIC BRIEF - THE REINFORCED FIX */
    .reason-box { 
    ...
    /* Explicitly targeting every possible text tag to force Navy Blue */
    .brief-navy h3, .brief-navy p, .brief-navy b, .brief-navy span, .brief-navy div, .brief-navy hr {
        color: #002366 !important;
    }

    /* HANDOVER LOG FIX */
    [data-testid="stTextArea"] textarea { color: #002366 !important; background-color: #ffffff !important; font-weight: bold !important; border: 2px solid #d6001a !important; }
...
# 7. BACKGROUND FETCH (STANDALONE LOGIC TO PREVENT TFS/GCTS FALSE TSRA)
...
st_folium(m, width=1200, height=1200, key="map_final_v247")
...
# 11. STRATEGY BRIEF (MULTI-LEVEL NAVY LOCK)
if st.session_state.investigate_iata != "None":
    ...
    st.markdown(f"""
    <div class="reason-box brief-navy">
        <h3 style="margin-top:0;">{iata} Strategy Brief</h3>
        <div style="font-size:1.15rem;">
            <p><b>Active Hazards:</b> <span>{issue_desc}</span>. Live X-Wind <b>{xw_val}kt</b>.</p>
            <p><b>Strategic Alternate:</b> <b style="color:#d6001a !important;">{alt_iata}</b> at <span>{min_dist} NM</span>.</p>
        </div>
        <hr>
        <div style="display:flex; gap:30px;">
            <div style="flex:1; padding:15px; background:#f9f9f9; border-radius:5px; border-left:4px solid #002366;">
                <b style="display:block; margin-bottom:10px; font-size:1.1rem !important;">LIVE METAR</b>
                <div style="font-family:monospace; line-height: 1.4 !important;">{bold_hazard(d.get('raw_m'))}</div>
            </div>
            <div style="flex:1; padding:15px; background:#f9f9f9; border-radius:5px; border-left:4px solid #002366;">
                <b style="display:block; margin-bottom:10px; font-size:1.1rem !important;">LIVE TAF</b>
...
st.text_area("Handover Report:", value=h_txt, height=200, label_visibility="collapsed")import streamlit as st
import folium
from streamlit_folium import st_folium
from avwx import Metar, Taf
import math
