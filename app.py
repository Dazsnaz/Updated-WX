        ...
        try:
            ...
            if t.data:
                for line in t.data.forecast:
                    ...
                    line_issues = []
                    if info['fleet'] == "Cityflyer" and ("FZRA" in line.raw or "FZDZ" in line.raw): line_issues.append("CLOSED-FZRA")
...
for iata, data in weather_data.items():
    ...
    v_lim, c_lim = (1500, 500) if info['spec'] else (800, 200)
    color = "#008000"
    actual_alert_str = "STABLE"
    forecast_alert_str = "NIL"
    ...
    if data['status'] == "online":
        ...
        if info['fleet'] == "Cityflyer" and ("FZRA" in data['raw_m'] or "FZDZ" in data['raw_m']):
            ...
            m_issues.append("MIN"); color = "#d6001a"
        elif data['vis'] < (v_lim * 2) or data['cig'] < (c_lim * 2):
            m_issues.append("MARG"); color = "#eb8f34"
        if xw > 25: m_issues.append("X-WIND"); color = "#eb8f34"
        
        if m_issues: 
            actual_alert_str = " / ".join(m_issues)
            metar_alerts[iata] = {"type": actual_alert_str, "hex": "primary" if color == "#d6001a" else "secondary"}
        else: 
            green_stations.append(iata)
            
        if data['f_issues']:
            p_tag = " (PROB)" if data['f_prob'] else ""
            forecast_alert_str = f"{' + '.join(data['f_issues'])}{p_tag} @ {data['f_time']}"
    ...
    <div style="width:500px; color:black !important; font-family:monospace; font-size:12px;">
        <b style="color:#002366; font-size:14px;">{iata} - {info['fleet']}</b>
        <div style="margin-top:5px; padding:5px; border:1px solid #ddd; border-left:5px solid {color}; background:#f9f9f9;">
            <b>ACTUAL:</b> <span style="color:#d6001a;">{actual_alert_str}</span><br>
            <b>FORECAST:</b> <span style="color:#eb8f34;">{forecast_alert_str}</span>
        </div>
        <hr>
